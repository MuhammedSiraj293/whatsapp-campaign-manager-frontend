...
export default function TemplateAnalytics() {
  ...
  if (isLoading) {
    return <p className="bg-gray-900 text-center text-gray-400">Loading template analytics...</p>;
  }

  if (!analytics) {
    return <p className="text-center text-red-500">Could not load analytics for this template.</p>;
  }
  ...
  return (
    <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-black min-h-screen w-full p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard title="Total" value={analytics.total} className="border-l-4 border-blue-500" />
        ...
        <StatCard title="Failed" value={analytics.failed} className="border-l-4 border-red-500" />
      </div>
    </div>
  );
}