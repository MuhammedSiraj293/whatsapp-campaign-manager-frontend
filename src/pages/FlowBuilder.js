...
export default function FlowBuilder() {
  ...
  const onConnect = useCallback(
    (params) => {
      setEdges((eds) => {
        ...
        return addEdge({ ...params, animated: true, style: { stroke: '#10b981', strokeWidth: 2 } }, filteredEds);
      });
    },
    [setEdges]
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await authFetch(`/bot-flows/${flowId}/nodes`);
      if (!response.success) throw new Error('Failed to fetch current nodes for sync.');
      
      const existingBackendNodes = response.data;
      const currentFrontendNodeIds = new Set(nodes.map(n => n.data._id).filter(id => id)); // Only existing IDs

      // 2. Identify nodes to delete (exist in backend but not in frontend)
      const nodesToDelete = existingBackendNodes.filter(node => !currentFrontendNodeIds.has(node._id));

      // 3. Delete removed nodes
      if (nodesToDelete.length > 0) {
        console.log('Deleting nodes:', nodesToDelete);
        await Promise.all(nodesToDelete.map(node => 
      ...
      const backendNodes = nodes.map((node) => {
        ...
        if (node.data.messageType === 'text') {
          ...
          const edge = connectedEdges.find(e => !e.sourceHandle || e.sourceHandle === 'null');
        ...
        if (node.data.messageType === 'buttons') {
          buttons = buttons.map((btn, index) => {
            const edge = connectedEdges.find(e => e.sourceHandle === `handle-btn-${index}`);
            return { ...btn, nextNodeId: edge ? getTargetNodeId(edge.target) : '' };
          });
        }
        ...
        if (node.data.messageType === 'list') {
          ...
          const updatedListItems = listItems.map((item, index) => {
            const edge = connectedEdges.find(e => e.sourceHandle === `handle-list-${index}`);
            return { ...item, nextNodeId: edge ? getTargetNodeId(edge.target) : '' };
          });
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center h-screen bg-[#0b141a] text-white">Loading Flow...</div>;
  }
      ...
      <div className="h-[calc(100vh-64px)] flex flex-col">
        ...
        <div className="bg-[#202d33] p-4 border-b border-gray-700 flex justify-between items-center">
          ...
          <div className="flex gap-2">
              <button onClick={onLayout} className="bg-[#2a3942] text-white px-4 py-2 rounded hover:bg-[#364852] flex items-center gap-2">
                  <FaMagic /> Auto Layout
              </button>
          ...
          <ReactFlowProvider>
            ...
            <div className="flex-1 h-full relative" ref={reactFlowWrapper}>
              ...
              >
                ...
                {selectedNode && (
                  <Panel position="top-right" className="bg-[#202d33] p-4 rounded-lg border border-gray-700 w-80 m-4 text-white shadow-xl max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="space-y-4">
                      ...
                      {selectedNode.data.messageType === 'text' && (
                          <div>
                              <label className="block text-xs text-gray-400 mb-1">Save Reply To (Variable)</label>
                              <input 
                                  type="text" 
                                  value={selectedNode.data.saveToField || ''} 
                                  onChange={(e) => updateNodeData('saveToField', e.target.value)}
                                  className="w-full bg-[#111b21] border border-gray-700 rounded p-2 text-sm text-white"
                              ...
                              <div className="space-y-2">
                                  {(selectedNode.data.buttons || []).map((btn, index) => (
                                      <div key={index} className="flex gap-2">
                                          ...
                                          />
                                          <button onClick={() => removeButton(index)} className="text-red-500 hover:text-red-400 p-2">
                                              &times;
                                          </button>
                                      </div>
                                  ))}
                                  <button onClick={addButton} className="w-full bg-[#2a3942] hover:bg-[#364852] text-emerald-500 text-sm py-2 rounded border border-dashed border-gray-600">
                                      + Add Button
                                  </button>
                              </div>
                          </div>
                      ...
                      {selectedNode.data.messageType === 'list' && (
                          <div>
                              <div className="mb-3">
                                  <label className="block text-xs text-gray-400 mb-1">Menu Button Text</label>
                                  <input 
                                      type="text" 
                              ...
                              <label className="block text-xs text-gray-400 mb-1">Menu Items</label>
                              <div className="space-y-2">
                                  {(selectedNode.data.listItems || []).map((item, index) => (
                                      <div key={index} className="flex flex-col gap-1 bg-[#111b21] p-2 rounded border border-gray-700">
                                          <div className="flex gap-2">
                                              <input 
                                                  type="text" 
                                                  value={item.title} 
                                              ...
                                              />
                                              <button onClick={() => removeListItem(index)} className="text-red-500 hover:text-red-400">
                                                  &times;
                                              </button>
                                          </div>
                                          <input 
                                  ...
                                  <button onClick={addListItem} className="w-full bg-[#2a3942] hover:bg-[#364852] text-orange-500 text-sm py-2 rounded border border-dashed border-gray-600">
                                      + Add Menu Item
                                  </button>
                              </div>
                          </div>
                      ...
                      <div className="pt-4 border-t border-gray-700 flex justify-between items-center">
                          <button onClick={deleteNode} className="text-red-500 hover:text-red-400 text-sm flex items-center gap-2">
                              <FaTrash /> Delete Node
                          </button>
                      </div>
                      ...
                      <div className="pt-2">
                          <p className="text-xs text-gray-500">
                              To connect nodes, drag from the <span className="text-emerald-500">green handle</span> on the right of a node to the left handle of another node.
                          </p>
                      </div>
                    </div>
                  </Panel>
                ...
                {/* Selected Edge Panel */}
                {selectedEdge && (
                  <Panel position="top-right" className="bg-[#202d33] p-4 rounded-lg border border-gray-700 w-64 m-4 text-white shadow-xl">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-bold text-lg">Connection</h3>
                      <button onClick={() => setSelectedEdge(null)} className="text-gray-400 hover:text-white">&times;</button>
                    </div>
                    <p className="text-sm text-gray-400 mb-4">
                      Selected connection from <strong>{selectedEdge.source}</strong> to <strong>{selectedEdge.target}</strong>.
                    </p>
                    <button onClick={deleteEdge} className="w-full bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 flex items-center justify-center gap-2">
