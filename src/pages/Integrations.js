  ...
  const handleSaveMasterSheetId = async (accountId) => {
    try {
      await authFetch(`/waba/accounts/${accountId}`, {
        method: 'PUT',
        body: JSON.stringify({ masterSpreadsheetId: editingSheetId[accountId] }),
      });
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="bg-[#202d33] p-6 rounded-lg shadow-lg">
          ...
          <form onSubmit={handleAddAccount} className="flex flex-col gap-4">
            <input type="text" value={accountName} onChange={(e) => setAccountName(e.target.value)}
              placeholder="Account Name (e.g., Client A)" className={inputStyle} required />

            <input type="text" value={accessToken} onChange={(e) => setAccessToken(e.target.value)}
              placeholder="Permanent Access Token" className={inputStyle} required />

            <input type="text" value={businessAccountId} onChange={(e) => setBusinessAccountId(e.target.value)}
              placeholder="Business Account ID" className={inputStyle} required />
        ...
        <div className="bg-[#202d33] p-6 rounded-lg shadow-lg">
          <h2 className="text-xl font-bold text-white mb-4">
            Add Phone Number to: {activeWaba ? accounts.find(a => a._id === activeWaba)?.accountName : '...'}
          </h2>
          ...
          <form onSubmit={handleAddPhone} className="flex flex-col gap-4">
            <input type="text" value={phoneName} onChange={(e) => setPhoneName(e.target.value)}
              placeholder="Phone Number Name" className={inputStyle} disabled={!activeWaba} required />

            <input type="text" value={phoneId} onChange={(e) => setPhoneId(e.target.value)}
              placeholder="Phone Number ID (from Meta)" className={inputStyle} disabled={!activeWaba} required />

            <button className={buttonStyle} disabled={!activeWaba}>Add Phone</button>
          </form>
        </div>
      </div>
              ...
              <div key={account._id} className="bg-[#2a3942] p-4 rounded-lg">
                ...
                <div className="mt-4">
                  ...
                  <div className="flex gap-2">
                    <input type="text"
                      ...
                      onChange={(e) => setEditingSheetId({ ...editingSheetId, [account._id]: e.target.value })}
                      className={inputStyle} />

                    <button onClick={() => handleSaveMasterSheetId(account._id)}
                ...
                <div className="mt-4">
                  ...
                  {account.phoneNumbers.map(phone => (
                    <div key={phone._id} className="bg-[#202d33] p-3 rounded mb-2">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => handleDeletePhone(phone._id)}
                      ...
                      <div className="mt-3">
                        ...
                        <div className="flex gap-2">
                          <select
                            ...
                            onChange={(e) => handleBotFlowChange(phone._id, e.target.value)}
                            className={inputStyle}
                          >
                            <option value="">-- No Bot --</option>

                            {botFlows
                              .filter(flow => flow.wabaAccount === account._id)
                              .map(flow => (
                          ...
                          <button onClick={() => handleSaveBotFlow(phone._id)}
                            className="text-white bg-sky-600 hover:bg-sky-700 rounded-lg p-2.5">
                            <FaSave />
                          </button>
                        </div>
                  ...
                  {account.phoneNumbers.length === 0 && (
                    <p className="text-xs text-gray-500 mt-2">No phone numbers yet.</p>
                  )}
                </div>
